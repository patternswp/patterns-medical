/*CSS*/
/* global PatternsMedicalLocalize */
...
/* Local */
class PatternsMedicalSettingsApi {
	constructor() {
		this.endpoint = 'patterns-medical/v1/settings';
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-medical-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-medical-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-medical-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-medical-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patterns-medical-processing' );
					this.installPlugins();
				}
			}
		} );
	...
	isLoadReview() {
		const removeReviewNoticePermanently =
			this.settings.remove_review_notice_permanently;
		const removeReviewNoticeTemporaryDateTime =
			this.settings.remove_review_notice_temporary_date_time;
		...
		const daysSinceInstallation =
			( currentDateTime - themeInstalledDateTime ) / ( 60 * 60 * 24 );
		...
		const daysSinceTemporary =
			( currentDateTime - removeReviewNoticeTemporaryDateTime ) /
			( 60 * 60 * 24 );
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-medical-review-notice'
			);
	...
	async installPlugins() {
		const plugins = PatternsMedicalLocalize.recommended_plugins;
		...
		window.location.replace( PatternsMedicalLocalize.theme_info_url );
	}
}

document.addEventListener( 'DOMContentLoaded', () => {
	new PatternsMedicalSettingsApi();
} );
